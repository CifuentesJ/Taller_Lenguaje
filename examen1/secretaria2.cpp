...
int main() {
    string *infra2021, *infra2022, clave, aux;
    int X, Y, pos2021 = 0;
    ...
    for(int i = 0; i < Y-1 && band; i++){
        band = false;
        for (int j = 0; j < Y-i-1; j++){
            if (infra2022[j].compare(infra2022[j+1])>0){
                aux = infra2022[j];
                infra2022[j] = infra2022[j+1];
                infra2022[j+1] = aux;
                band = true;
            }
        }
    ...
    for (int i = 0; i < X; i++){
     ...
     int  izq = 0, der = Y-1, mit = (izq + der)/2;

    while(clave != infra2022[mit] && izq<der){
        if(clave>infra2022[mit]){
            izq = mit + 1;
        }else{
            der = mit - 1;
        }
    ...
    if(infra2022[mit] == clave){
        band2022 = true;
    }
    
    if(band2021 && band2022){
        cout << "DEBE SER MULTADO E INMOVILIZADO. PLACA ENCONTRADA EN POSICIONES " <<  pos2021 << " Y " << mit << "."<< endl;
    }else if (band2021){
        cout << "DEBE SER MULTADO. PLACA ENCONTRADA EN POSICION " <<  pos2021 << " EN EL ARREGLO INFRA2021." << endl;
    } else if (band2022){
        cout << "DEBE SER MULTADO. PLACA ENCONTRADA EN POSICION " <<  mit << " EN EL ARREGLO INFRA2022." << endl;
    }else{
